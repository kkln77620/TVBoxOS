...
//        getWindow().setFlags(WindowManager.LayoutParams.FLAG_LAYOUT_NO_LIMITS, WindowManager.LayoutParams.FLAG_LAYOUT_NO_LIMITS);
    ...
    protected static Drawable globalWp = null;

    /**
     * 内置壁纸列表(按顺序切换)
     */
    public static final int[] BUILTIN_WALLPAPERS = {
            R.drawable.wallpaper_1, R.drawable.wallpaper_2, R.drawable.wallpaper_3,
            R.drawable.wallpaper_4, R.drawable.wallpaper_5, R.drawable.app_bg
    };

    /**
     * 重置壁纸缓存(供"恢复默认壁纸"调用)
     */
    public static void resetWallpaperCache() {
        globalWp = null;
    }

    public void changeWallpaper(boolean force) {
        String mode = Hawk.get(HawkConfig.WALLPAPER_MODE, "builtin");
        ...
        try {
            if ("builtin".equals(mode)) {
                // 内置壁纸: 按序号循环
                int idx = Hawk.get(HawkConfig.WALLPAPER_INDEX, 0);
                int resId = BUILTIN_WALLPAPERS[Math.floorMod(idx, BUILTIN_WALLPAPERS.length)];
                globalWp = ContextCompat.getDrawable(this, resId);
            } else if ("upload".equals(mode)) {
                // 上传壁纸(仅图片): 采样解码
                String path = Hawk.get(HawkConfig.WALLPAPER_PATH, "");
                File wp = new File(path);
                if (wp.exists() && wp.length() > 0) {
                    ...
                    if (opts.outWidth <= 0 || opts.outHeight <= 0) {
                        ...
                        android.graphics.Bitmap bmp = BitmapFactory.decodeFile(wp.getAbsolutePath(), opts);
                        if (bmp != null) {
                            globalWp = new BitmapDrawable(getResources(), bmp);
                        } else {
                            wp.delete();
                        }
                    }
