...
//        mGridView.setLayoutManager(new V7GridLayoutManager(this.mContext, isBaseOnWidth() ? 6 : 7));
    ...
    private int getCurrentPlayIndex() {
        try {
            if (vodInfo == null) return 0;
            return vodInfo.getplayIndex();
        } catch (Throwable th) {
            return 0;
        }
    }
    ...
    private String getCurrentPlayUrl() {
        try {
            ...
            int idx = getCurrentPlayIndex();
            if (idx < 0 || idx >= series.size()) return null;
            VodInfo.VodSeries vs = series.get(idx);
            return vs == null ? null : vs.url;
        } catch (Throwable th) {
            return null;
        }
    }
