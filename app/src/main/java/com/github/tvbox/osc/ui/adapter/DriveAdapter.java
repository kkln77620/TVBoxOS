...
public class DriveAdapter extends BaseQuickAdapter<DriveFolderFile, BaseViewHolder> {
        ...
        if(item.isDrive())
        {
            ...
            if(item.isFile) {
                ...
                if(StorageDriveType.isVideoType(item.fileType)) {
                    ...
                    // 本地视频: 异步提取首帧作为预览图
                    if(item.getDriveData() != null && item.getDriveType() == StorageDriveType.TYPE.LOCAL) {
                        loadVideoThumb(imgItem, getLocalFilePath(item));
                    }
                } else if(StorageDriveType.isAudioType(item.fileType)) {
                    imgItem.setImageResource(R.drawable.v_audio);
                } else {
    ...
    /**
     * 构造本地文件完整路径: 网盘根目录 + 访问路径 + 文件名
     */
    private String getLocalFilePath(DriveFolderFile item) {
        try {
            String base = item.getDriveData() != null ? item.getDriveData().name : "";
            return base + item.getAccessingPathStr() + item.name;
        } catch (Exception e) {
            return null;
        }
    }
    ...
    /**
     * 异步提取本地视频首帧作为预览图, 失败时保留原图标
     */
    private void loadVideoThumb(ImageView iv, String path) {
